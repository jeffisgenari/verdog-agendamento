...
export default function NovoAnuncioForm() {
  ...
  const [localHospedagem, setLocalHospedagem] = useState<"CASA_PROFISSIONAL" | "CASA_DONO">(
    "CASA_PROFISSIONAL"
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 px-4 pt-4 pb-8">
      ...
      {tipoServico === "HOSPEDAGEM" && (
        <label className="text-xs text-neutral-600">
          ...
          <select
            ...
            onChange={(e) => setLocalHospedagem(e.target.value as "CASA_PROFISSIONAL" | "CASA_DONO")}
            className="w-full mt-1 border border-neutral-200 rounded-lg px-3 py-2 text-sm"
          >
            <option value="CASA_PROFISSIONAL">Na minha casa</option>
            <option value="CASA_DONO">Na casa do dono do pet</option>
          </select>
        </label>
      )}
      ...
      <div className="border-t border-neutral-100 pt-3 mt-1">
        ...
        <p className="text-xs text-neutral-500 mt-0.5">Até {MAX_FOTOS} fotos.</p>
